...
def upgrade() -> None:
    ...
    op.create_table('food_characteristic',
    ...
    sa.Column('ingredient_price_modifier', sa.Numeric(precision=10, scale=4), nullable=False),
    ...
    op.create_table('restaurant_food_disabled',
    sa.Column('restaurant_id', sa.Integer(), nullable=True),
    sa.Column('food_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['food_id'], ['food.id'], ),
    sa.ForeignKeyConstraint(['restaurant_id'], ['restaurant.id'], )
    )
    op.create_table('food_characteristic_variant',
    sa.Column('characteristic_id', sa.Integer(), nullable=True),
    sa.Column('variant_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['characteristic_id'], ['food_characteristic.id'], ),
    sa.ForeignKeyConstraint(['variant_id'], ['food_variant.id'], )
    )
    ...
    op.drop_constraint('menu_category_restaurant_id_fkey', 'menu_category', type_='foreignkey')
...
def downgrade() -> None:
    ...
    op.add_column('menu_category', sa.Column('restaurant_id', sa.INTEGER(), autoincrement=False, nullable=False))
    op.create_foreign_key('menu_category_restaurant_id_fkey', 'menu_category', 'restaurant', ['restaurant_id'], ['id'])
    op.add_column('food', sa.Column('food_type', sa.VARCHAR(length=200), autoincrement=False, nullable=True))
    op.add_column('food', sa.Column('ingredient_price_modifier', sa.NUMERIC(precision=10, scale=4), autoincrement=False, nullable=False))
    op.add_column('food', sa.Column('sub_name', sa.VARCHAR(length=200), autoincrement=False, nullable=True))
    op.add_column('food', sa.Column('is_active', sa.BOOLEAN(), autoincrement=False, nullable=False))
    op.add_column('food', sa.Column('price', sa.NUMERIC(precision=10, scale=2), autoincrement=False, nullable=False))
    ...
    sa.ForeignKeyConstraint(['category_id'], ['menu_category.id'], name='category_food_category_id_fkey'),
    sa.ForeignKeyConstraint(['food_id'], ['food.id'], name='category_food_food_id_fkey')
    )
