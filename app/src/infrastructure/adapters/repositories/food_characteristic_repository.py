...
class FoodCharacteristicRepository(IFoodVariantRepository):
    ...
    async def add_characteristics_to_variant_by_id(self, variant_id: int, characteristic_value: str) -> FoodCharacteristic:
        stmt = select(FoodVariant).where(FoodVariant.id == variant_id)
        ...
        if characteristic:
            variant.characteristics.append(result_char.scalars().first())
        else:
